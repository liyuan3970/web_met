...
def sync_station_data():
    ...
    for table in station_dict.keys():
        # 查询气象站数据
        query = f"""
        select r1.IIiii as station_no, r1.station_name as station_name, r1.lon as lon, r1.lat as lat, r1.rain as p_total, r1.tmax as t_max, r1.tmin as t_min, r2.wind_max as w_max, r2.wind_dir as w_dir, r1.vis as vis from
        (select st.IIiii as IIiii, max(st.StationName) as station_name, max(st.lon) as lon, max(st.lat) as lat, sum(if(sd.RR>0,sd.RR,0)) as rain, max(if(sd.T>-2730,sd.T,null)) as tmax, min(if(sd.T>-2730,sd.T,null)) as tmin, min(if(sd.VV>=0,sd.VV,null)) as vis from {station_table} as st
        ...
        if results:
            ...
            values_list = [(date, data["station_no"], data["station_name"], data["lon"], data["lat"], station_type,
                            data["p_total"], data["t_max"], data["t_min"], data["w_max"], data["w_dir"], data["vis"])
                           for data in results]

            # 处理数据
            upsert_sql = f"""
            INSERT INTO station_data(datetime, station_no, station_name, lon, lat, station_type, p_total, t_max, t_min, w_max, w_dir, vis) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) 
            ON DUPLICATE KEY UPDATE datetime = VALUES(datetime), station_no = VALUES(station_no), station_name = VALUES(station_name),
